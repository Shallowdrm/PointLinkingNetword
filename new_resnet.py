...
class InceptionResNetV2(nn.Module):
    ...
    def forward(self, input):
        ...
        # x = self.softmax(x)
        # x_part = x[:,:,:,3:17]
        # x_part = F.softmax(x_part,dim=-1,_stacklevel=5)
        # x[:,:,:,3:17] = x_part
        # x[:,:,:,3 + 51 * 1:17 + 51 * 1] = self.softmax(x[:,:,:,3 + 51 * 1:17 + 51 * 1])
        # x[:,:,:,3 + 51 * 2:17 + 51 * 2] = self.softmax(x[:,:,:,3 + 51 * 2:17 + 51 * 2])
        # x[:,:,:,3 + 51 * 3:17 + 51 * 3] = self.softmax(x[:,:,:,3 + 51 * 3:17 + 51 * 3])
        # x[:,:,:,17:31] = self.softmax(x[:,:,:,17:31])
        # x[:,:,:,17 + 51 * 1:31 + 51 * 1] = self.softmax(x[:,:,:,17 + 51 * 1:31 + 51 * 1])
        # x[:,:,:,17 + 51 * 2:31 + 51 * 2] = self.softmax(x[:,:,:,17 + 51 * 2:31 + 51 * 2])
        # x[:,:,:,17 + 51 * 3:31 + 51 * 3] = self.softmax(x[:,:,:,17 + 51 * 3:31 + 51 * 3])
        # x[:,:,:,31:51] = self.softmax(x[:,:,:,31:51])
        # x[:,:,:,31 + 51 * 1:51 + 51 * 1] = self.softmax(x[:,:,:,31 + 51 * 1:51 + 51 * 1])
        # x[:,:,:,31 + 51 * 2:51 + 51 * 2] = self.softmax(x[:,:,:,31 + 51 * 2:51 + 51 * 2])
        # x[:,:,:,31 + 51 * 3:51 + 51 * 3] = self.softmax(x[:,:,:,31 + 51 * 3:51 + 51 * 3])
        # x = self.logits(x)
        return x
...
def pretrained_inception():
    ...
    model_dict = model.state_dict()
    pretrained_dict = torch.load("E:\学习\Pycharm\文件\CV\learn\PLN\inceptionresnetv2-520b38e4.pth")
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
    model_dict.update(pretrained_dict)
    model.load_state_dict(model_dict)
    return model
